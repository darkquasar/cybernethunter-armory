                    ...
                    if($SUCCESS -eq 0)
                    {
                        if($ProtocolStatus -eq 0)
                        {
                            ...
                            if ($Global:OnlyReturnTicketHashes) {
                                ...
                                $EncodedTicketSHA256Hex = -join ([byte[]] $EncodedTicketSHA256Bytes).ForEach('ToString', 'X2')
                            }
                            ...
                            if ($Global:OnlyReturnTicketHashes) {
                                $obj | Add-Member -MemberType NoteProperty -Name SessionKeySHA256 -Value $SessionKeySHA256Hex
                            }
                            else {
                            ...
                            if ($Global:OnlyReturnTicketHashes) {
                                $obj | Add-Member -MemberType NoteProperty -Name EncodedTicketSHA256 -Value $EncodedTicketSHA256Hex
                            }
                            else {
...
Function Start-DumpKerberosTicketsMetadata {

    <#

    .SYNOPSIS
        Simple helper function to dump contents of Get-KerberosTicketGrantingTicket to a json file
    ...
    Param (
        ...
        [switch]$DumpTicketToFile,

        [Parameter(Mandatory=$False)]
        [string]$OutputFilePath,

        [Parameter(Mandatory=$False)]
        [switch]$OnlyReturnTicketHashes,
		
		[Parameter(Mandatory=$False)]
        [int]$RemoveTicketsFileAfterSeconds
		
    )

    if ($OnlyReturnTicketHashes) {
        $Global:OnlyReturnTicketHashes = $True
    }

    # Get Tickets
    $tgt = Get-KerberosTicketGrantingTicket
    ...
    if ($DumpTicketToFile) {
        ...
        try {
            # Setup Log Output
            $date = Get-Date
            $identifier = "$($env:COMPUTERNAME)-$($date.Year)$($date.Month)$($date.Day)"
            $OutputFilePath = "$OutputFilePath\$identifier-kerberos-tickets.json"
        }
        catch {
            Write-Host "Could not dump ticket to file"
        }
    }
    ...
    # Since it contains sensitive information, should we delete the kerberos tickets file after a certain amount of time? 
    if ($RemoveTicketsFileAfterSeconds) {
        Start-Sleep $RemoveTicketsFileAfter
