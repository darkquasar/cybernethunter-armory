...
# Initiate Logger
$ProfileLogger = [Logger]::New()
$ProfileLogger.LogMessage("Initiating Profile Loading", "INFO", $null, $null)

# Source a different profile if required
# $NewProfileFile = "NewLocation\Microsoft.PowerShell_profile.ps1"
# . $NewProfileFile

# Remove SMB module dependencies to reduce lag
[System.Collections.ArrayList]$ClearedModulePath = $env:PSModulePath.Split(";")
$env:PSModulePath.Split(";") | ForEach-Object { if($_ -match "\\\\"){$ClearedModulePath.remove($_)} }
$env:PSModulePath = $ClearedModulePath -join ";"
$ProfileLogger.LogMessage("Filtered PSModulePath to: $env:PSModulePath", "INFO", $null, $null)

# Getting the current profile path
$ProfilePath = (Get-ItemProperty 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -Name Personal).Personal
$ProfileLogger.LogMessage("Home Path as per Registry: $ProfilePath", "INFO", $null, $null)
